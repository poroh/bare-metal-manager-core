...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let credential_provider = Arc::new(TestCredentialProvider::new(fallback_credentials.clone()));
    ...
    if let Some(iterations) = args.benchmark {
        ...
        for _ in 0..iterations {
            explorer
                .generate_exploration_report(bmc_ip_address, fallback_credentials.clone(), None)
                .await?;
        }
